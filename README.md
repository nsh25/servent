...
optimization:
  ...
  # Candidate params may include: min_spike_candles, min_body_to_atr,
  # min_directional_ratio, gap_mode/gap_threshold, min/max correction depth,
  # max_bars_after_spike, second_leg_mode, entry_mode, stop_buffer, rr_target,
  # htf/session filters
  max_trials: 30
  top_n_results_to_save: 5
```
...
- `walk_forward_selected_params.csv` (top-N parameter sets per fold)
- `walk_forward_optimization_summary.csv` (in-sample best vs out-of-sample comparison per fold)
...
### Leakage safeguards
- Train and test windows are strictly non-overlapping by index (`train_end <= test_start`).
- Parameter optimization runs only on train data per fold and selected parameters are frozen for the following test fold.
- Final performance estimate in `walk_forward_summary.csv` uses only out-of-sample fold results.
- Warmup bars are used only to initialize indicators before test bars; only trades entered in the test interval are counted in OOS metrics.

### Rolling vs expanding
- `rolling_window`: fixed-size train window that slides forward.
- `expanding_window`: train window always starts at the first bar and grows over time.
