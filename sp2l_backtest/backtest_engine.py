...
from .replay import SetupReplay
...
class BacktestEngine:
    ...
    def run(self, df: pd.DataFrame | None = None) -> Dict[str, object]:
        ...
        if self.config.walk_forward.walk_forward_enabled:
            wf_results = WalkForwardRunner(self.config, prepared, output_dir).run()
            return {"walk_forward": wf_results}
        ...
        if self.config.walkthrough.walkthrough_enabled and self.config.walkthrough.walkthrough_setup_id is not None:
            replay = SetupReplay(self.config, output_dir)
            results["walkthrough"] = replay.replay_setup(prepared, results, self.config.walkthrough.walkthrough_setup_id)
        return results
